...
Deno.serve(async (req) => {
  ...
  if (!body.code || typeof body.code !== "string") {
    return jsonResponse({ error: "Missing lobby code" }, 400);
  }
  ...
  if (!isValidLobbyCodeFormat(normalizedCode)) {
    return jsonResponse({ error: "Invalid lobby code format" }, 400);
  }
  ...
  if (lobbyError) {
    return jsonResponse({ error: "Failed to find lobby" }, 500);
  }
  ...
  if (lobby.status !== "waiting") {
    return jsonResponse({ error: "Lobby is not accepting players" }, 403);
  }
  ...
  if (countError) {
    return jsonResponse({ error: "Failed to check lobby capacity" }, 500);
  }

  if ((playerCount ?? 0) >= lobby.max_players) {
    return jsonResponse({ error: "Lobby is full" }, 409);
  }
  ...
  if (duplicateNameError) {
    return jsonResponse({ error: "Failed to check display name" }, 500);
  }

  if (duplicateName) {
    return jsonResponse({ error: "Display name is already taken in this lobby" }, 409);
  }
  ...
  if (existingPlayer) {
    ...
    if (updatePlayerError) {
      return jsonResponse({ error: "Failed to join lobby" }, 500);
    }
  } else {
