...
Deno.serve(async (req) => {
  ...
  const { data: existingProgress, error: progressSelectError } = await supabase
    ...
    .select("id, scored_char_indices, phrase_bonus_awarded, finalized")
  ...
  if (scoreResult.pointsAwarded === 0 && !scoreResult.phrasesCompletedDelta) {
    const { data: playerRow } = await supabase
      .from("players")
      .select("score, phrases_completed")
      .eq("id", player.id)
  ...
  if (scoreResult.pointsAwarded > 0) {
    try {
      await broadcastScoreUpdate(supabase, lobby.id, {
        ...
        score: newScore,
        phrases_completed: newPhrasesCompleted,
        delta: scoreResult.pointsAwarded,
      });
    } catch {
